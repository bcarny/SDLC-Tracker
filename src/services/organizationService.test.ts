...
describe('organizationService', () => {
  ...
  describe('list', () => {
    it('returns organizations from repository', async () => {
      const orgs = [
        { id: 'org-1', name: 'Org 1', description: null, createdAt: new Date(), updatedAt: new Date() },
        { id: 'org-2', name: 'Org 2', description: null, createdAt: new Date(), updatedAt: new Date() },
      ]
  ...
  describe('getById', () => {
    ...
    it('throws when organization not found', async () => {
      ...
      await expect(organizationService.getById('missing')).rejects.toThrow('Organization not found')
  ...
  describe('create', () => {
    ...
    it('passes optional description', async () => {
      vi.mocked(repo.create).mockResolvedValue({ id: 'org-1', name: 'O', description: 'Desc', createdAt: new Date(), updatedAt: new Date() } as never)
  ...
  describe('update', () => {
    it('updates when org exists', async () => {
      const existing = { id: 'org-1', name: 'Old', description: null, createdAt: new Date(), updatedAt: new Date(), applications: [], teams: [] }
    ...
    it('throws when organization not found', async () => {
      ...
      await expect(organizationService.update('missing', { name: 'New' })).rejects.toThrow('Organization not found')
  ...
  describe('delete', () => {
    it('deletes when org exists', async () => {
      const existing = { id: 'org-1', name: 'Org', description: null, createdAt: new Date(), updatedAt: new Date(), applications: [], teams: [] }
      vi.mocked(repo.findById).mockResolvedValue(existing as never)
